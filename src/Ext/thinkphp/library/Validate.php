...
namespace app\ext\thinkphp\library;
...
class Validate
{
    ...
    /**
     * @var $isRemainRuleField bool 是否保留只存在 rule 的字段。默认 false
     */
    protected $isRemainRuleField = false;

    /**
     * @var $convertRuleStyle string 转换字段风格。camelCase 小驼峰，snake_case 蛇形
     */
    protected $convertFieldStyle;
    ...
    public function check(&$data, $rules = [], $scene = '')
    {
        ...
        $data = $this->convertFieldStyleExec($data);
     ...
     */
    protected function convertFieldStyleExec($data)
    {
        if ($this->convertFieldStyle === null) {
            return $data;
        }
        else {
            $output = [];
            $stringStyleTool = CodeStyleTool::getInstance();
            foreach ($data as $key => $item) {
                $output[$stringStyleTool->to($key, $this->convertFieldStyle)] = $item;
            }
            return $output;
        }
    }
    ...
    protected function checkItem($field, $value, $rules, $data, $title = '', $msg = [])
    {
        ...
        foreach ($rules as $key => $rule) {
            ...
            if ($rule instanceof \Closure) {
                ...
                // 如果不是 require 有数据才会行验证 或自定义验证
                if (0 === strpos($info, 'require') || (!is_null($value) && '' !== $value) || 0 === strpos($info, 'custom')) {
                    // 验证类型
                    $callback = isset(self::$type[$type]) ? self::$type[$type] : [$this, $type];
                    // 验证数据
